...
export default function EcommercePage() {
  ...
  const handleAdd = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setItems(prev => [{ id: Date.now().toString(), name: f.get("name") as string, desc: f.get("desc") as string, status: "Online" }, ...prev]);
  ...
  return (
    <div className="h-full flex flex-col gap-5">
      <div className="flex items-center justify-between shrink-0">
        <div>
          <h1 className="text-2xl font-bold text-white tracking-tight">E-commerce e Delivery</h1>
          <p className="text-slate-500 text-sm mt-0.5">Integra vendas online, aplicativos de entrega e sincroniza estoque e pedidos.</p>
        </div>
        ...
        <div className="flex-1 overflow-auto min-h-0">
          {items.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full">
               <ShoppingBag size={48} className="mx-auto text-indigo-500/20 mb-4" />
            ...
            <table className="w-full text-sm text-left">
              <thead className="bg-[#0c0f1a]/60 border-b border-indigo-500/[0.08] text-slate-500 sticky top-0">
                <tr>
                  <th className="px-5 py-3 font-semibold uppercase tracking-wider text-[10px]">Plataforma</th>
                  <th className="px-5 py-3 font-semibold uppercase tracking-wider text-[10px]">Chave API / Detalhes</th>
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-[#111528] border border-indigo-500/15 rounded-2xl w-full max-w-md shadow-2xl overflow-hidden">
            <div className="p-5 border-b border-indigo-500/[0.08] flex justify-between items-center">
              <h3 className="text-lg font-bold text-white">Adicionar Integração</h3>
            ...
            <form onSubmit={handleAdd} className="p-5 space-y-4">
              <div>
                <label className="block text-xs font-semibold text-slate-400 mb-1">Plataforma (ex: iFood)</label>
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-400 mb-1">Detalhes de Conexão</label>
