...
export default function AuditPage() {
  ...
  const handleAdd = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setItems(prev => [{ id: Date.now().toString(), name: f.get("name") as string, desc: f.get("desc") as string, status: "Revisado" }, ...prev]);
  ...
  return (
    <div className="h-full flex flex-col gap-5">
      <div className="flex items-center justify-between shrink-0">
        <div>
          <h1 className="text-2xl font-bold text-white tracking-tight">Auditoria e Segurança</h1>
          <p className="text-slate-500 text-sm mt-0.5">Controla acessos, permissões e registra todas as operações realizadas no sistema.</p>
        </div>
        ...
        <div className="flex-1 overflow-auto min-h-0">
          {items.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full">
               <ShieldCheck size={48} className="mx-auto text-indigo-500/20 mb-4" />
            ...
            <table className="w-full text-sm text-left">
              <thead className="bg-[#0c0f1a]/60 border-b border-indigo-500/[0.08] text-slate-500 sticky top-0">
                <tr>
                  <th className="px-5 py-3 font-semibold uppercase tracking-wider text-[10px]">Ação / Usuário</th>
                  <th className="px-5 py-3 font-semibold uppercase tracking-wider text-[10px]">Detalhes da Operação</th>
              ...
              <tbody className="divide-y divide-indigo-500/[0.06]">
                {filtered.map(i => (
                  <tr key={i.id} className="hover:bg-indigo-500/[0.04]">
                    ...
                    <td className="px-5 py-3"><span className="text-[10px] px-2 py-0.5 bg-indigo-500/10 text-indigo-400 border border-indigo-500/15 font-bold rounded-md">{i.status}</span></td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-[#111528] border border-indigo-500/15 rounded-2xl w-full max-w-md shadow-2xl overflow-hidden">
            <div className="p-5 border-b border-indigo-500/[0.08] flex justify-between items-center">
              <h3 className="text-lg font-bold text-white">Criar Registro Manual</h3>
            ...
            <form onSubmit={handleAdd} className="p-5 space-y-4">
              <div>
                <label className="block text-xs font-semibold text-slate-400 mb-1">Ação</label>
